  ...
  class DiscountFixed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VoucherifySdk::DiscountFixed` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VoucherifySdk::DiscountFixed`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @fixed_amount.nil?
        invalid_properties.push('invalid value for "fixed_amount", fixed_amount cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @fixed_amount.nil?
      true
    end
    ...
    def hash
      [type, fixed_amount, fixed_amount_formula, effect, is_dynamic].hash
    end
