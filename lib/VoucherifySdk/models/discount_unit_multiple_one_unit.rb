  ...
  class DiscountUnitMultipleOneUnit
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VoucherifySdk::DiscountUnitMultipleOneUnit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VoucherifySdk::DiscountUnitMultipleOneUnit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      effect_validator = EnumAttributeValidator.new('String', ["ADD_NEW_ITEMS", "ADD_MISSING_ITEMS"])
    ...
    def effect=(effect)
      validator = EnumAttributeValidator.new('String', ["ADD_NEW_ITEMS", "ADD_MISSING_ITEMS"])
    ...
    def hash
      [unit_off, unit_off_formula, effect, unit_type, product, sku].hash
    end
