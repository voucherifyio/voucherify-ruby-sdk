...
module VoucherifySdk
  class ListPublicationsItemInvalid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VoucherifySdk::ListPublicationsItemInvalid` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VoucherifySdk::ListPublicationsItemInvalid`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      result_validator = EnumAttributeValidator.new('String', ["FAILURE"])
    ...
    def result=(result)
      validator = EnumAttributeValidator.new('String', ["FAILURE"])
    ...
    def hash
      [id, object, created_at, customer_id, tracking_id, metadata, channel, source_id, customer, vouchers, vouchers_id, result, failure_code, failure_message].hash
    end
