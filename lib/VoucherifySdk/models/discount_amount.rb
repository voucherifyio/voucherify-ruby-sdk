  ...
  class DiscountAmount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VoucherifySdk::DiscountAmount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VoucherifySdk::DiscountAmount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @amount_off.nil?
        invalid_properties.push('invalid value for "amount_off", amount_off cannot be nil.')
      end
    ...
    def hash
      [type, amount_off, amount_off_formula, aggregated_amount_limit, effect, is_dynamic].hash
    end
