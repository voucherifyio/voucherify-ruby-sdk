...
module VoucherifySdk
  class CampaignsCreatePromotionCampaign
    # Campaign name.
    attr_accessor :name
    ...
    def self.openapi_types
      {
        ...
        :'promotion' => :'SchemaThatContainsUniquePropertiesForPromotionCampaignPromotion'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VoucherifySdk::CampaignsCreatePromotionCampaign` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VoucherifySdk::CampaignsCreatePromotionCampaign`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      campaign_type_validator = EnumAttributeValidator.new('String', ["PROMOTION"])
    ...
    def campaign_type=(campaign_type)
      validator = EnumAttributeValidator.new('String', ["PROMOTION"])
    ...
    def hash
      [name, description, type, join_once, auto_join, use_voucher_metadata_schema, vouchers_count, start_date, expiration_date, validity_timeframe, validity_day_of_week, validity_hours, activity_duration_after_publishing, validation_rules, category_id, category, metadata, campaign_type, promotion].hash
    end
